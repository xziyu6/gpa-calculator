...
def save_input():
    ...
    if os.path.splitext(path)[1] == '.xlsx':
        ...
        save_input_label.config(text='Invalid file, required: .xlsx')
...
def save_gpa():
    ...
    output_path = os.path.join(os.path.dirname(path), 'GPA output.txt')
    ...
    with open(output_path, 'w') as fout:
        fout.write(current_content + content + '\n')
    save_gpa_label.config(text='GPA saved at: ' + os.path.abspath(output_path))
