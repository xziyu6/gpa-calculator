...
# respond to saveInputBtn, save input
def saveInputCom():
    info = getInput()
    path = tk.filedialog.asksaveasfilename(initialdir=".\\",
                                           title="Save as Excel file",
                                           filetypes=(("Microsoft Excel", "*.xlsx"),
                                                      ("all files", "*.*")),
                                           defaultextension=(("Microsoft Excel", "*.xlsx"),
                                                             ("all files", "*.*")))
    info.to_excel(path)


# respond to saveGpaBtn, save GPA
def saveGpaCom():
    gpa = outLbl["text"].split(": ")[1]
...
# info entries
widgetGroup = pd.DataFrame({}, index=sbjList,
                           columns=["sbjLbl", "levelEty", "scoreEty", "creditEty", "levelLbl", "scoreLbl", "creditLbl"])
for i in range(7):
...
# save input
saveInputBtn = tk.Button(text="Save input", command=saveInputCom)
saveInputBtn.grid(row=9, column=2, columnspan=2, pady=2)

# save GPA
saveGpaBtn = tk.Button(text="Save GPA", command=saveGpaCom)
saveGpaBtn.grid(row=9, column=4, columnspan=2, pady=2)
